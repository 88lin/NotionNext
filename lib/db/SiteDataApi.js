    ...
    allPages: [
      {
        ...
        pageCoverThumbnail: BLOG.HOME_BANNER_IMAGE || 'https://cdn.jsdmirror.com/gh/88lin/picx-images-hosting@master/chiiikawa.64e0o4q2po.webp',
        date: {
...
export async function resolvePostProps({
  ...
  if (post && post.id && !post?.blockMap) {
    try {
      ...
      post.blockMap = unwrapNotionBlockMap(rawBlockMap)
 ...
 * 修复新版 Notion 返回的包裹结构
 * 规则：
 * - 只要对象形如 { spaceId | space_id, value }
 * - 就解包为 { value }
 * - 不拍平 value，不修改内部结构
 */
export function unwrapNotionBlockMap(blockMap) {
  ...
  const unwrapMap = (map) => {
    ...
    for (const [id, item] of Object.entries(map)) {
      ...
      if (hasSpaceId && item.value) {
        fixed[id] = { value: item.value }
      } else {
        fixed[id] = item
      }
    }
  ...
  // 🔑 对 blockMap 下的所有一级对象统一处理
  for (const key of Object.keys(blockMap)) {
    result[key] = unwrapMap(blockMap[key])
  }
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || '茉灵智库'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || '网站维护中...博主正在积极抢救'
  const defaultPageCover = NOTION_CONFIG?.HOME_BANNER_IMAGE || 'https://cdn.jsdmirror.com/gh/88lin/picx-images-hosting@master/chiiikawa.64e0o4q2po.webp'
  const defaultIcon = NOTION_CONFIG?.AVATAR || '/zz.svg'
